...
### 2.5 文章粒度：一篇 = 一个独立完整的知识点（用户明确要求）

写/改文章时**不要按笔记的章节结构简单分组**，按"知识点"切分：

- **一篇讲一个独立、自足的知识点**——读完这篇就能掌握它，不必跳到别的篇补前因后果。
- **太碎的要合并**：同一知识群的多个小算法合为一篇，用二级标题分节。
  例：冒泡+快速 → 交换排序；顺序/折半/分块查找 → 线性查找三兄弟；DFS+BFS → 图的遍历。
- **太大的要拆开**：一篇塞了多个独立知识点（多为整章搬运）时按知识点拆。
  例：《定点数的表示与运算》(1.1 万字 / 12 节) → 表示 / 移位与加减 / 乘除与运算器 三篇。
- **参考区间**：正文 3000~8000 字。低于 3000 字先看能否并进同族篇目；高于 12000 字且含多个独立知识点则拆。
- **判据是自足性而非字数**：自成体系的算法（KMP、红黑树）即便短也可独立成篇；一个完整流程被切成前后两半才叫碎。

> ⚠️ **合并/删除文章会改变 permalink**（`/blog/YYYY/MM/DD/<文件名>/`），旧链接 404。
> 结构调整后必须：① 同步 `source/mindmaps/*.md` 的文章链接；② 重跑 `python gen_mindmaps.py`
> （计算机网络导图的 html 需单独跑 `npx -y markmap-cli ...`）。
